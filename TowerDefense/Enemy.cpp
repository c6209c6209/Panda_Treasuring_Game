...
Enemy::Enemy(std::string img, float x, float y, float w, float h, float radius, float speed) :
	Engine::Sprite(img, x, y, w, h), speed(speed) {
...
void Enemy::Update(float deltaTime) {
	...
	for (auto& it : scene->PlayerGroup->GetObjects()) {
		Panda* panda = dynamic_cast<Panda*>(it);
		if (!panda->Visible)
			continue;
		if (Engine::Collider::IsCircleOverlap(Position, CollisionRadius, panda->Position, panda->CollisionRadius)) {
		    panda->OnExplode();
			getPlayScene()->Hit();
			getPlayScene()->EnemyGroup->RemoveObject(objectIterator);
			return;
